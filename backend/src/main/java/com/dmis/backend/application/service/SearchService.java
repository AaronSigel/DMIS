...
public class SearchService {
    ...
    public SearchService(ChunkSearchPort chunkSearchPort, AclService aclService, LlmChatPort llmChatPort, AuditService auditService) {
    ...
    public SearchDtos.SearchResponse search(UserView actor, String query) {
        ...
        List<SearchDtos.SearchHitView> hits = chunkSearchPort.search(actor.id(), isAdmin, query, 10).stream()
                .map(hit -> new SearchDtos.SearchHitView(hit.documentId(), hit.title(), hit.chunkId(), hit.chunkText(), hit.score()))
                .toList();
